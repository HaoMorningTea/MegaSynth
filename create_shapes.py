# Zeroverse code release: https://github.com/desaixie/zeroverse/blob/main/create_shapes.py
...
class Shape(object):
    ...
    def genObj(self, filePath, mat_path, bMat=False, bComputeNormal=False, bScaleMesh=False, bMaxDimRange=[0.3, 0.5]):
        ...
        if bScaleMesh:
            ...
            # print(minP, maxP)
            offset = (maxP + minP) / 2.0
            self.points = self.points - offset.reshape((1,3))
            # breakpoint()
            minP = np.min(self.points, axis=0)
            maxP = np.max(self.points, axis=0)
            print('Scale to [%f, %f] from [%f, %f]\n' % (-max_dim, max_dim, np.min(minP), np.max(maxP)))
...
def createVarObjShapes(outFolder, shapeIds, mat_path, uuid_str='', sub_obj_nums=[1, 2, 3, 4, 5, 6, 7, 8, 9], sub_obj_num_poss=[1, 2, 3, 7, 10, 7, 3, 2, 1],
    ...
    for ii, i in enumerate(shapeIds):  # for each MultiShape
        ...
        max_dim, material_ids = ms.genObj(subFolder + "/object.obj", mat_path=mat_path, bMat=True, bComputeNormal=True, bScaleMesh=bScaleMesh, bMaxDimRange=bMaxDimRange)
